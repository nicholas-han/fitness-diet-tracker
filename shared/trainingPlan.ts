...
export const DEFAULT_WEEKLY_SCHEDULE: WeeklyScheduleEntry[] = [
  ...
  { id: "saturday", dayIndex: 5, day: "周六", title: "Strength C", type: "strength", programId: "strength-c", secondary: "可选：拳击或网球", optionalTasks: [{ id: "saturday-boxing", title: "拳击", type: "boxing", sessionType: "Other", secondary: "可选 conditioning，不计硬性达标" }, { id: "saturday-optional-tennis", title: "额外网球", type: "tennis", sessionType: "Mixed", secondary: "可选第三次网球，按恢复情况决定" }] },
...
export function migrateWeeklySchedule(schedule: WeeklyScheduleEntry[]) {
  return schedule.map(entry => {
    if (entry.optionalTasks !== undefined) return entry;
    const defaultEntry = DEFAULT_WEEKLY_SCHEDULE.find(candidate => candidate.id === entry.id || candidate.dayIndex === entry.dayIndex);
    return defaultEntry?.optionalTasks ? { ...entry, optionalTasks: defaultEntry.optionalTasks.map(task => ({ ...task })) } : entry;
  });
}
