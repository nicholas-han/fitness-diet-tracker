...
import { movingAverage, rollingAverage, trendChange, useFitnessStore } from "@/lib/localStore";
  ...
  return <div className="pb-10">
    ...
    {tab === "recovery" && <div className="space-y-6">
      <Section title="Recovery trends" meta="优先看滚动平均和多日变化"><div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        <Panel><p className="text-xs text-muted-foreground">睡眠（7 日平均）</p><p className="mt-1 text-2xl font-semibold">{display(rollingAverage(state.recovery, 7, entry => entry.sleepHours), " h")}</p><p className="text-xs text-muted-foreground">变化 {signed(trendChange(state.recovery, 7, entry => entry.sleepHours))} h</p></Panel>
        <Panel><p className="text-xs text-muted-foreground">HRV（7 日平均）</p><p className="mt-1 text-2xl font-semibold">{display(rollingAverage(state.recovery, 7, entry => entry.hrv))}</p><p className="text-xs text-muted-foreground">变化 {signed(trendChange(state.recovery, 7, entry => entry.hrv))}</p></Panel>
        <Panel><p className="text-xs text-muted-foreground">静息心率（7 日平均）</p><p className="mt-1 text-2xl font-semibold">{display(rollingAverage(state.recovery, 7, entry => entry.restingHr), " bpm")}</p><p className="text-xs text-muted-foreground">变化 {signed(trendChange(state.recovery, 7, entry => entry.restingHr))} bpm</p></Panel>
        <Panel><p className="text-xs text-muted-foreground">WHOOP Recovery（7 日平均）</p><p className="mt-1 text-2xl font-semibold">{display(rollingAverage(state.recovery, 7, entry => entry.whoopRecovery))}</p><p className="text-xs text-muted-foreground">变化 {signed(trendChange(state.recovery, 7, entry => entry.whoopRecovery))}</p></Panel>
      </div></Section>
