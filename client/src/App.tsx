...
export function parseRiceCupGrams(value: string): number | undefined {
...
function RiceCupPrompt() {
  ...
  const value = parseRiceCupGrams(draft);
  const save = () => {
    if (value === undefined) return;
    update(current => ({ ...current, settings: { ...current.settings, riceCupGrams: value, riceCupPromptDismissed: true } }));
  };
  return <div className="fixed inset-x-3 bottom-4 z-40 mx-auto flex max-w-xl flex-wrap items-center gap-3 rounded-lg border border-amber-500/40 bg-card p-3 shadow-lg"><div className="min-w-0 flex-1"><p className="text-sm font-medium">校准电饭煲米杯</p><p className="mt-1 text-xs text-muted-foreground">填写一杯生米的克数，Log 才能准确换算杯数。</p></div><input aria-label="米饭一杯生重" type="number" min="1" placeholder="克" value={draft} className="w-20 rounded border border-input bg-background px-2 py-1.5 text-sm" onChange={event => setDraft(event.target.value)} onKeyDown={event => { if (event.key === "Enter") save(); }} /><button type="button" disabled={value === undefined} onClick={save} className="rounded bg-primary px-3 py-1.5 text-xs font-medium text-primary-foreground disabled:opacity-50">保存</button><button type="button" onClick={() => update(current => ({ ...current, settings: { ...current.settings, riceCupPromptDismissed: true } }))} className="text-xs text-muted-foreground">稍后</button></div>;
}
